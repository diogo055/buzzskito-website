...
interface BlogCardProps {
  ...
  isPillar?: boolean
  /** Lifted-surface styling for the dark one-canvas pages. */
  dark?: boolean
}

export default function BlogCard({ slug, title, excerpt, date, category = 'mosquito', isPillar = false, dark = false }: BlogCardProps) {
  ...
  if (dark) {
    return (
      <article className="hover-lift surface-1 rounded-2xl overflow-hidden flex flex-col">
        <div className={`h-1 ${category === 'tick' ? 'bg-amber-400/70' : 'bg-brand-500/70'}`} aria-hidden="true" />
        <div className="p-6 flex flex-col flex-1">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-lg" aria-hidden="true">{icon}</span>
            <span className="text-xs font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-white/10 text-white/70">
              {categoryLabel}
            </span>
            {isPillar && (
              <span className="text-xs font-bold uppercase tracking-wide px-2 py-0.5 rounded-full bg-white/15 text-white">
                Pillar Guide
              </span>
            )}
          </div>
          <h3 className="text-lg font-bold text-white mb-2 leading-snug">{title}</h3>
          <p className="text-[#8fa0bd] text-sm leading-relaxed flex-1 mb-4">{excerpt}</p>
          <div className="flex items-center justify-between mt-auto">
            <time className="text-xs text-[#7688a8]" dateTime={date}>{formattedDate}</time>
            <Link
              href={`/blog/${slug}`}
              className="arrow-nudge text-sm font-semibold text-white/85 hover:text-white flex items-center gap-1"
