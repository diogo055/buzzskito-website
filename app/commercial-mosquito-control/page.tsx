...
export default function CommercialMosquitoPage() {
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(localBusinessSchema({ description: 'Commercial mosquito and tick barrier spray for businesses, restaurants, event venues, and managed properties across the Greater Toronto Area.' })) }} />
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(speakableSchema('/commercial-mosquito-control', '2026-07-01')) }} />

      <CityHero
        breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Mosquito Control', href: '/mosquito-control' }, { label: 'Commercial' }]}
        title={<>Commercial Mosquito &amp; Tick Control in the GTA</>}
        subtitle={<>Protect your guests, employees, and outdoor spaces from mosquitoes and ticks. BuzzSkito provides professional barrier spray for restaurants, event venues, golf courses, managed communities, and all commercial properties across the GTA.</>}
