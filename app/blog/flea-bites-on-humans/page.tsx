  ...
  return (
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 py-14 prose-brand">
        ...
        <p>The vacuum is the single most important piece of equipment in that list, and the machine matters more than people expect. You want strong suction at floor level, a beater bar that agitates carpet pile deep enough to lift eggs and larvae out of it, and a sealed bag you can lift straight out and put in an outdoor bin &mdash; a bagless canister means tipping live fleas, eggs and pupae out over your own rubbish bin every day for weeks. A hose and crevice tool for baseboards, carpet edges and the car matters as much as the head does. <BuyLink tag={AMZ_TAG} search="bagged vacuum pet hair carpet">Check bagged pet-hair vacuums on Amazon &rarr;</BuyLink></p>
        ...
        <p>The pupae are also why the steam step above is worth the trouble in the rooms where bites keep happening. A carpet steam cleaner puts heat and moisture down into the pile where the larvae feed and the cocoons sit, which is the one place a vacuum reaches mechanically but a spray largely does not &mdash; and the heat and vibration together prompt dormant pupae to hatch out into a room you are already vacuuming daily, which shortens the two-to-eight-week wait rather than extending it. Check the machine is rated for the flooring before you start, and let carpets dry fully afterwards. <BuyLink tag={AMZ_TAG} search="carpet cleaner steam machine">Check carpet steam cleaners on Amazon &rarr;</BuyLink></p>
        ...
        <p>If the evidence in your bedroom pointed at bed bugs rather than fleas, there is one piece of equipment worth owning before any spray: a zippered mattress and box-spring encasement. It seals whatever is already inside the mattress in, stops anything new getting into the seams and piping where the population concentrates, and turns the bed into a surface you can actually inspect &mdash; anything on white fabric on the outside is visible in seconds. Buy one labelled bed-bug-proof with a zipper stop rather than a plain dust-mite cover, and leave it on for at least a year. <BuyLink tag={AMZ_TAG} search="bed bug mattress encasement zippered">Check mattress encasements on Amazon &rarr;</BuyLink></p>
        <div className="overflow-x-auto not-prose my-6">
          <table className="min-w-[560px] w-full text-sm border-collapse border border-navy-100 rounded-lg overflow-hidden">
