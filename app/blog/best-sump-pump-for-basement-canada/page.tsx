    ...
    <>
      ...
      <article className="py-12 px-4 bg-white">
        <div className="max-w-3xl mx-auto prose-brand">
          ...
          <p>The other half of that problem is that a failed switch is <em>silent</em>. Nothing announces it; the pit simply fills while the house sleeps, and you find out when the carpet is wet. A battery water alarm sitting on the floor beside the pit, or clipped an inch above the normal high-water line, turns that into a noise at 2 a.m. instead of a claim in the morning. It is the cheapest item in this entire guide. <BuyLink tag={AMZ_TAG} search="basement water alarm leak detector">Check basement water alarms on Amazon.ca &rarr;</BuyLink></p>
          ...
          <p>Ice is only one of the two ways a winter storm takes a pump out. The other is the power cut that arrives with it, usually at the exact hour the pit is filling fastest, and no amount of correct slope helps a pump with no electricity. A battery backup unit sits in the same basin as a second pump on its own DC circuit and covers that gap. On an unfinished utility basement it is optional; under a finished floor it is the difference between an inconvenience and a renovation. <BuyLink tag={AMZ_TAG} search="sump pump battery backup system">Check battery backup systems on Amazon.ca &rarr;</BuyLink></p>
          ...
          <p>If you want to know whether any of this is working, measure it rather than guess. A cheap hygrometer left on a shelf near the pit tells you what the basement&rsquo;s relative humidity actually is, and that single number is what decides whether silverfish and sowbugs can complete a life cycle down there. Seal the pit, then watch the reading fall over the following week &mdash; that is the pest fix showing up as evidence. <BuyLink tag={AMZ_TAG} search="indoor humidity monitor hygrometer">Check humidity monitors on Amazon.ca &rarr;</BuyLink></p>
          ...
          <p>Extending that run is the rare job where the part costs less than the trip to fetch it. A corrugated discharge extension or a rigid pipe kit with an elbow gets the outlet past the saturated zone, and a splash pad stops the far end digging its own puddle &mdash; which would simply move the mosquito habitat six feet and call it solved. <BuyLink tag={AMZ_TAG} search="sump pump discharge hose kit">Check discharge hose and extension kits on Amazon.ca &rarr;</BuyLink></p>
