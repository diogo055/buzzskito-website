    ...
    <>
      ...
      <article className="py-12 px-4 bg-white">
        <div className="max-w-3xl mx-auto prose-brand">
          ...
          <p>Of those four routes, the one most households do not already own equipment for is <strong>steam</strong> &mdash; and it is the route that covers the things you cannot put in a machine: the wool area rug, the stair runner, the upholstered chair frame, the closet floor edge. A handheld garment steamer is enough for upholstery and runners; a floor steam cleaner earns its keep only if you have wall-to-wall wool. The technique is the same either way and it is not intuitive: move slowly, in overlapping passes, so the heat has time to go <em>into</em> the pile rather than across it, test a hidden corner for colourfastness first, and open a window so the item dries fully afterwards rather than staying damp. <BuyLink tag={AMZ_TAG} search="handheld fabric steamer upholstery">check handheld fabric steamers on Amazon.ca &rarr;</BuyLink></p>
          ...
          <p>Totes solve the folded half of the problem. The hanging half &mdash; coats, suits, anything wool that creases badly &mdash; needs a <strong>zippered garment bag</strong> instead, and the two details that matter are a full-length zip that closes all the way to the hook and a solid rather than mesh panel at the bottom, which is where an open-bottomed dust cover lets everything straight back in. Cotton or canvas bags breathe, which suits a damp basement closet better than sealed vinyl; vinyl suits a dry upstairs closet. Either way the item goes in clean, because zipping an unworn-since-March coat in with its own body-oil residue is the mistake this whole step exists to prevent. <BuyLink tag={AMZ_TAG} search="zippered garment storage bag">check zippered garment bags on Amazon.ca &rarr;</BuyLink></p>
          ...
          <p>Take the screens seriously, because a torn screen on the one window beside a flowering shrub is doing more work against you than anything else on the list. You almost never need a new screen: a roll of replacement mesh, a spline roller and a few metres of spline rescreens a whole frame in about fifteen minutes at the kitchen table, and self-adhesive patch squares handle a single tear in a frame you cannot get out. Fibreglass mesh is the easy material to work with; aluminium holds up better where a dog or a cat leans on it. <BuyLink tag={AMZ_TAG} search="window screen repair kit mesh spline">check window screen repair kits on Amazon.ca &rarr;</BuyLink></p>
          <div className="not-prose my-4">
