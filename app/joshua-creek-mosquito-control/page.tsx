...
export const metadata: Metadata = buildMetadata({
  title: 'Mosquito Control Joshua Creek | From $99',
  description:
    'Professional mosquito control in Joshua Creek, Oakville. Barrier spray near Joshua Creek ravine from $99. Licensed Ontario operator. Call (289) 216-5030.',
  canonical: '/joshua-creek-mosquito-control',
})

const NEIGHBOURHOOD = 'Joshua Creek'
const CITY = 'Oakville'
const SLUG = '/joshua-creek-mosquito-control'

const FAQS = [
  {
    question: 'Why does the Joshua Creek neighbourhood have so many mosquitoes?',
    answer: "The Joshua Creek neighbourhood is named after the creek that runs directly through it — and that creek is the primary reason for the area's persistent mosquito pressure. Joshua Creek's ravine system provides every condition mosquitoes need: permanent water flow with standing pools, saturated floodplain soils, dense riparian vegetation for daytime resting, and a sheltered microclimate that retains moisture and humidity even during dry spells.",
  },
  {
    question: 'Which parts of Joshua Creek have the worst mosquito activity?',
    answer: "Properties that back directly onto the Joshua Creek ravine system experience the highest mosquito pressure — particularly along the ravine's east and west banks where mature vegetation provides abundant resting habitat. Streets near the creek's tributary channels and stormwater outfalls also see elevated activity. However, mosquitoes disperse up to 3 km from their breeding sites, so even properties several blocks from the ravine experience meaningful pressure throughout the season.",
  },
  {
    question: 'How many treatments does a Joshua Creek property need?',
    answer: "Season plans run May through September: Basic (5 sprays, monthly), Standard (10 sprays, every 2 weeks) or Exclusive (20+ sprays, weekly). For properties adjacent to the Joshua Creek ravine, we typically recommend Standard's two-week spacing; the ravine's permanent water and dense vegetation mean consistent mosquito production throughout the season. Properties farther from the ravine may be well-served by Basic. Call (289) 216-5030 for a free assessment.",
  },
  {
    question: 'How does BuzzSkito treat properties next to the Joshua Creek ravine?',
    answer: "Our spray is applied only to vegetation on your private property — not in the ravine itself or near waterways — and according to label directions, which set out how close to water it may be used. We target leaf undersides, shrub interiors, and fence lines where mosquitoes rest. Treatments are applied by an Ontario-licensed pesticide operator (Licence L-240-2436835197). Keep children and pets off treated areas until the spray has dried, as the product label directs.",
  },
  {
    question: 'Does BuzzSkito serve all of the Joshua Creek neighbourhood?',
    answer: "Yes. We serve all streets in the Joshua Creek neighbourhood, including properties with direct ravine frontage. We also serve all surrounding Oakville neighbourhoods. Call (289) 216-5030 to confirm service at your specific address.",
  },
]

export default function JoshuaCreekMosquitoPage() {
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(speakableSchema('/joshua-creek-mosquito-control', '2026-07-01')) }} />
      ...
      <CityHero
        ...
        subtitle={<>Licensed Ontario pesticide operator. Treatments applied according to label directions, for Joshua Creek homes near the ravine system that runs through the neighbourhood.</>}
      ...
      <TypicalPrices service="mosquito" city="Joshua Creek" />
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-4xl mx-auto prose-brand">
          <h2>Living with Joshua Creek's Mosquito Pressure</h2>
          <p>The Joshua Creek neighbourhood takes its name from the creek that defines its landscape — and that same waterway is the primary driver of the area's mosquito activity. The <strong>Joshua Creek ravine system</strong> provides a textbook mosquito breeding environment: permanent water flow with standing pools along its banks, saturated floodplain soils that retain water after every rainfall, and dense riparian vegetation that shelters adult mosquitoes during daylight hours.</p>
          <p>Properties backing directly onto the ravine experience the most intense pressure, but mosquito dispersal means the entire neighbourhood is affected. BuzzSkito's barrier spray creates a treated perimeter on your property by targeting every surface where arriving mosquitoes rest — leaf undersides, shrub interiors, fence lines, and other vegetation. This interrupts the cycle between the ravine source and your outdoor living space, knocking down what is resting on that vegetation and leaving a residual behind. The ravine keeps producing new mosquitoes, so the treatment is repeated through the season rather than done once.</p>

          <h2>Part of Our Oakville Mosquito Control Network</h2>
          <p>We serve all of Joshua Creek and surrounding Oakville. See our <Link href="/oakville-mosquito-control" className="text-brand-700 hover:underline">Oakville mosquito control</Link> page for city-wide coverage.</p>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <CityPriceCard city="Joshua Creek" service="mosquito" location="price_card_faq" />
      </div>
      ...
      <CTASection heading={`Get a Free Mosquito Control Quote in ${NEIGHBOURHOOD}`} subtext="Protecting Joshua Creek properties from ravine mosquitoes. No contracts." />
    </>
  )
}
